...
m <- leaflet(USWater) %>%
	...
	addCircles(stroke = TRUE,  color = ~colorNumeric( palette = "YlOrRd", Ave, n = 9)(Ave))
m %>% addProviderTiles(providers$CartoDB.Positron) %>%

addLegend("bottomleft", pal = pal, values = ~Ave,
    title = "Est. Per Capita Drinking Water Consumption Average 1995-2005",
